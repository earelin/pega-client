const COLOR_PARTIDOS: Record<string, string> = {
    BNG: '#6AADE5',
    CIUDADANOS: '#FE5000',
    PP: '#17589D',
    PODEMOS: '#9169F4',
...
export function partidoColor(nome: string): string {
    const nomeLower = nome.toLowerCase();

    if (nomeLower.includes('ciudadanos')) {
        return COLOR_PARTIDOS.CIUDADANOS;
    }

    if (nomeLower.includes('vox')) {
        return COLOR_PARTIDOS.VOX;
    }

    if (nomeLower.includes('partido popular')) {
        return COLOR_PARTIDOS.PP;
    }

    if (nomeLower.includes('partido socialista obrero español')) {
        return COLOR_PARTIDOS.PSOE;
    }

    if (nomeLower.includes('podemos')) {
